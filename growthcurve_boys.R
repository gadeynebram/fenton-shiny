...
ggplot(data = boys_all_gather %>% dplyr::filter(type == "weight"), aes(x = `Compl weeks`, y = as.numeric(value), linetype = annotation, shape = type))  + 
...
ggplot(data = boys_all_gather %>% dplyr::filter(type  %in% c("HC","length") ), aes(x = `Compl weeks`, y = as.numeric(value), linetype = annotation, col = type))  + 
